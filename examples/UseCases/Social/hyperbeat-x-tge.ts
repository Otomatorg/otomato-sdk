...
async function hyperbeat() {
  ...
  aiAiAction.setParams('prompt', 'the tweet mentions TGE');
  ...
  telegramSendMessageAction.setParams('message', `HyperBeat protocol alert: a tweet mentionning TGE has been posted ${xXPostTriggerTrigger.getOutputVariableName('tweetURL')}`);
  telegramSendMessageAction.setPosition(400, 480);
  
  const edge1 = new Edge({ source: xXPostTriggerTrigger, target: aiAiAction });
  const edge2 = new Edge({ source: conditionIfAction, target: telegramSendMessageAction });
  const edge3 = new Edge({ source: aiAiAction, target: conditionIfAction });
  
  const workflow = new Workflow('Get notified when Hyperbeat mentions TGE', [xXPostTriggerTrigger, aiAiAction, conditionIfAction, telegramSendMessageAction], [edge1, edge2, edge3], null);
