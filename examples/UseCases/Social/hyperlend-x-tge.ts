...
async function hyperlend() {
  ...
  telegramSendMessageAction.setParams('message', `Hyperlend protocol alert: a tweet mentionning TGE has been posted ${xXPostTriggerTrigger.getOutputVariableName('tweetURL')}`);
  ...
  const workflow = new Workflow('Get notified when Hyperlend mentions TGE', [xXPostTriggerTrigger, aiAiAction, conditionIfAction, telegramSendMessageAction], [edge1, edge2, edge3], null);
