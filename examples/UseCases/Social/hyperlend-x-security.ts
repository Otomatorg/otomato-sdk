...
async function hyperlend() {
  ...
  telegramSendMessageAction.setParams('message', `Hyperlend protocol alert: a tweet mentionning security alerts ${xXPostTriggerTrigger.getOutputVariableName('tweetURL')}`);
  ...
  const workflow = new Workflow('Get notified when Hyperlend a tweet mentionning security alerts', [xXPostTriggerTrigger, aiAiAction, conditionIfAction, telegramSendMessageAction], [edge1, edge2, edge3], null);
