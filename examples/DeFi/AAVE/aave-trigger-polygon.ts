...
async function aave_trigger_polygon() {
  ...
  aaveTrigger.setChainId(CHAINS.POLYGON);
  aaveTrigger.setParams(
    "abiParams.asset",
    getTokenFromSymbol(CHAINS.POLYGON, "USDC.e").contractAddress  // DAI, EURS, USDC.e, WPOL, USDC, USDT, WETH
  );
  aaveTrigger.setParams(
    "condition",
    "gt"
  ...
  console.log("AAVE Trigger Polygon before: " + workflow.getState());

  console.log("Workflow ID: " + workflow.id);

  const runResult = await workflow.run();

  console.log("AAVE Trigger Polygon after: " + workflow.getState());
}

aave_trigger_polygon();