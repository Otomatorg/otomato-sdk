...
async function aave_trigger_ethereum() {
  ...
  aaveTrigger.setParams(
    "abiParams.asset",
    getTokenFromSymbol(CHAINS.ETHEREUM, "FRAX").contractAddress  // DAI, ENS, SNX, RPL, LUSD, PYUSD, FRAX
  );
  aaveTrigger.setParams(
    "condition",
    "gt"
  ...
  console.log("AAVE Trigger Ethereum before: " + workflow.getState());

  console.log("Workflow ID: " + workflow.id);

  const runResult = await workflow.run();

  console.log("AAVE Trigger Ethereum after: " + workflow.getState());
}

aave_trigger_ethereum();