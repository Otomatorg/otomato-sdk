...
async function aave_trigger_sonic() {
  ...
  aaveTrigger.setChainId(CHAINS.SONIC);
  aaveTrigger.setParams(
    "abiParams.asset",
    getTokenFromSymbol(CHAINS.SONIC, "USDC.e").contractAddress  // wS, WETH, stS, USDC.e
  );
  aaveTrigger.setParams(
    "condition",
    "gt"
  ...
  console.log("AAVE Trigger Sonic before: " + workflow.getState());

  console.log("Workflow ID: " + workflow.id);

  const runResult = await workflow.run();

  console.log("AAVE Trigger Sonic after: " + workflow.getState());
}
