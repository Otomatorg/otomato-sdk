...
async function aave_trigger_arbitrum() {
  ...
  aaveTrigger.setChainId(CHAINS.ARBITRUM);
  aaveTrigger.setParams(
    "abiParams.asset",
    getTokenFromSymbol(CHAINS.ARBITRUM, "FRAX").contractAddress  // 
  );
  aaveTrigger.setParams(
    "condition",
    "gt"
  ...
  console.log("AAVE Trigger Arbitrum before: " + workflow.getState());

  console.log("Workflow ID: " + workflow.id);

  const runResult = await workflow.run();

  console.log("AAVE Trigger Arbitrum after: " + workflow.getState());
}

aave_trigger_arbitrum();