...
async function aave_trigger_avalanche() {
  ...
  aaveTrigger.setChainId(CHAINS.AVALANCHE);
  aaveTrigger.setParams(
    "abiParams.asset",
    getTokenFromSymbol(CHAINS.AVALANCHE, "WETH.e").contractAddress  // WAVAX, USDC, WETH, AUSD, DAI.e, LINK.e, AAVE.e
  );
  aaveTrigger.setParams(
    "condition",
    "gt"
  ...
  console.log("AAVE Trigger Avalanche before: " + workflow.getState());

  console.log("Workflow ID: " + workflow.id);

  const runResult = await workflow.run();

  console.log("AAVE Trigger Avalanche after: " + workflow.getState());
}

aave_trigger_avalanche();